...
# Download HF model files with curl (no --continue-at; proxy-safe).
# Pattern: download to <file>.part, then atomically rename. Skip if final exists.
...
dl() {  # url dest_dir file
  ...
  echo "  [dl] $file"
  rm -f "$dst/$file.part"
  if curl -sL --retry 4 --retry-delay 3 --max-time 3600 -o "$dst/$file.part" "$url"; then
...
for f in config.json pytorch_model.bin tokenizer.json tokenizer_config.json special_tokens_map.json sentencepiece.bpe.model; do
  dl "https://huggingface.co/BAAI/bge-m3/resolve/main/$f" "$BGE" "$f"
done
...
for f in config.json generation_config.json merges.txt model-00001-of-00002.safetensors model-00002-of-00002.safetensors model.safetensors.index.json tokenizer.json tokenizer_config.json vocab.json; do
  dl "https://huggingface.co/Qwen/Qwen2.5-7B-Instruct-AWQ/resolve/main/$f" "$QWEN" "$f"
done
